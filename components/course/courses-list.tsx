...
export function CoursesList({ courses, categories, enrolledCourseIds, isLoggedIn }: CoursesListProps) {
  ...
  const filteredCourses = useMemo(() => {
    ...
    return courses.filter((course) => course.categories?.some((cat) => categoryIds.includes(cat.id)));
    ...
    <div className="space-y-6">
      ...
      {filteredCourses.length === 0 ? (
        <Card>
          <CardHeader>
            <CardTitle>
              {selectedCategory ? "해당 카테고리에 등록된 강의가 없습니다" : "아직 등록된 강의가 없습니다"}
            </CardTitle>
            <CardDescription>
              {selectedCategory ? "다른 카테고리를 선택해 보세요" : "곧 새로운 강의가 추가될 예정입니다"}
            </CardDescription>
          </CardHeader>
        </Card>
      ) : (
          ...
          {filteredCourses.map((course) => {
            ...
            return (
              <Link
                key={course.id}
                href={
                  isEnrolled ? `/student/courses/${course.id}` : isLoggedIn ? `/courses/${course.id}` : "/auth/login"
                }
                className="group">
