...
export function ConversationListItem({ conversation, onClick }: ConversationListItemProps) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-200 cursor-pointer transition-colors text-left border-b ">
      {/* Avatar */}
      <Avatar className="h-12 w-12 shrink-0">
        <AvatarImage src={partner.avatar_url || undefined} alt={partner.display_name} />
        <AvatarFallback className="text-sm">{partner.display_name?.charAt(0) || "?"}</AvatarFallback>
      </Avatar>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="text-xs text-muted-foreground shrink-0">{formatRelativeTime(lastMessageAt)}</span>
        </div>
        <div className="flex items-center justify-between gap-2 mt-0.5">
          <p className="text-sm text-muted-foreground truncate">{lastMessage || "대화를 시작하세요"}</p>
          {unreadCount > 0 && (
